...
int inputAnInteger( int begin, int end )
{
   ...
   for( unsigned int i = 0; i < strlen( string ); i++ )
      if( string[ i ] < '0' || string[ i ] > '9' )//�ˬd�r�ꤤ���C�Ӧr���A�T�O�C�Ӧr�����O�Ʀr�r��
         return -1;
...
void makingReservation( int numSouthboundTrains, int numNorthboundTrains )
{
   ...
   do
   {
      ...
      do cout << "\n? ";
      while( ( reservation.destinationStation = inputAnInteger( 1, 12 ) ) == -1 );//��ܥX�o�Υت������A��X���~�h���s��J
...
bool suitable( Reservation &reservation, int numSouthboundTrains, int numNorthboundTrains )
{
   ...
   //�ˬd�_�W
        ...
        if (atoi(reservation.trainNumber) % 2 != 0) {
        ...
        cout << "Sorry, there is no suitable train for outbound time." << endl;
        return false;
    }
    //�ˬd�n�U
    // else if (reservation.originStation < reservation.destinationStation) {
...
void loadSouthboundTimetable( int &numSouthboundTrains )
{
    ...
    ifstream southtimetable("./Southbound timetable.txt", ios::in);
    if(!southtimetable) {
        cout << "Not find file.";
        return;
    }

    while (southtimetable >> southboundTimetable[numSouthboundTrains].trainNumber) {
        for (int i = 1; i < 13; i++) {
            southtimetable >> southboundTimetable[numSouthboundTrains].departureTimes[i];
        }
        numSouthboundTrains++;
    }
    southtimetable.close();
}
...
void loadNorthboundTimetable( int &numNorthboundTrains )
{
    ...
    ifstream northtimetable("./Northbound timetable.txt", ios::in);
    if (!northtimetable) {
        cout << "Not find file.";
        return;
    }
    while (northtimetable >> northboundTimetable[numNorthboundTrains].trainNumber) {
        for (int i = 1; i < 13; i++) {
            northtimetable >> northboundTimetable[numNorthboundTrains].departureTimes[i];
        }
        numNorthboundTrains++;
    }
    northtimetable.close();
...
void saveReservation( Reservation reservation )
{
    fstream outfile("./Reservation details.dat", ios::in | ios::app | ios::binary);
    
    outfile << reservation.idNumber << " " << reservation.reservationNumber << " " << reservation.trainNumber << " " << reservation.phone << " "
        << reservation.date << " " << reservation.originStation << " " << reservation.destinationStation << " " << reservation.carClass << " "
        << reservation.adultTickets << " " << reservation.concessionTickets << endl;
...
bool existReservation( fstream &ioFile, Reservation &reservation )
{
   ...
   while (ioFile >> reservation.idNumber ) {
       ioFile >> reservation.reservationNumber >> reservation.trainNumber >> reservation.phone >> reservation.date 
              >> reservation.originStation >> reservation.destinationStation >> reservation.carClass >> reservation.adultTickets
              >> reservation.concessionTickets;
       if (atoi(idNumber) != atoi(reservation.idNumber)) {
           continue;
       }
       else {
           if (atoi(reservationNumber) == atoi(reservation.reservationNumber)) {
               return true;
           }
       }
   }
   cout << "Reservation record does not exist or cannot be found in this system." << endl << endl;
   return false;
//    ioFile.close();
}
...
void displayReservation( Reservation reservation, Train trainTimetable[ 100 ], char stations[ 13 ][ 12 ] )
{
    ...
    cout << setw(10) << reservation.date
         << setw(10) << reservation.trainNumber
         << setw(12) << stations[reservation.originStation]
         << setw(12) << stations[reservation.destinationStation];
    for (int i = 0; i < 100; i++) {
        if (atoi(reservation.trainNumber) != atoi(trainTimetable[i].trainNumber)) {
            continue;
        }
    ...
    if (reservation.carClass == 1) {
        int fareAdult = adultTicketPrice[reservation.destinationStation][reservation.originStation] * reservation.adultTickets;
        int fareConcession = adultTicketPrice[reservation.destinationStation][reservation.originStation] / 2 * reservation.concessionTickets;
        int totalFare = fareAdult + fareConcession;
        cout << setw(7) << adultTicketPrice[reservation.destinationStation][reservation.originStation] << "*" << reservation.adultTickets;
        cout << setw(9) << adultTicketPrice[reservation.destinationStation][reservation.originStation] / 2 << "*" << reservation.concessionTickets;
        cout << setw(8) << totalFare;
    }

    else if (reservation.carClass == 2) {
        int fareAdult = adultTicketPrice[reservation.originStation][reservation.destinationStation] * reservation.adultTickets;
        int fareConcession = adultTicketPrice[reservation.originStation][reservation.destinationStation] / 2 * reservation.concessionTickets;
        int totalFare = fareAdult + fareConcession;
        cout << setw(7) << adultTicketPrice[reservation.originStation][reservation.destinationStation] << "*" << reservation.adultTickets;
        cout << setw(9) << adultTicketPrice[reservation.originStation][reservation.destinationStation] / 2 << "*" << reservation.concessionTickets;
        cout << setw(8) << totalFare;
    }
