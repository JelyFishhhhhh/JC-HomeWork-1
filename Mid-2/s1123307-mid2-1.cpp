...
int main()
{
    ...
    for (int i = 0; i < numTestCases; i++)
    {
        ...
        //delete[] quotient.terms; quotient�B�⦳��
...
void addition(Polynomial& addend, Polynomial adder)
{
    if (addend.degree < adder.degree) { //add
        addend.degree = adder.degree;
    }
    for (int i = 0; i <= addend.degree; i++) { 
        addend.terms[i] += (i<= adder.degree? adder.terms[i]: 0);
    }
    while (addend.terms[addend.degree] == 0 && addend.degree > 0) { //add
        addend.degree--;
    }
...
void multiplication(Polynomial multiplicand, Polynomial multiplier, Polynomial& product)
{
    ...
    product.terms = new int[product.degree + 1](); //add
    for (int i = 0; i <= multiplicand.degree; i++) { //modify
        for (int j = 0; j <= multiplier.degree; j++) { //modify
...
void division(Polynomial dividend, Polynomial divisor, Polynomial& quotient, Polynomial& remainder)
{
    ...
    remainder.terms = new int[remainder.degree + 1](); //modify
    ...
    quotient.terms = new int[quotient.degree + 1](); //modify
    //while (remainder.degree - divisor.degree >= 0) {
        for (int j = remainder.degree; j >= divisor.degree; j--) {

            quotient.terms[j - divisor.degree] = remainder.terms[remainder.degree] / divisor.terms[divisor.degree];
            monomial.terms[j - divisor.degree] = quotient.terms[j - divisor.degree];
            monomial.degree = j - divisor.degree; //add
            buffer.degree = divisor.degree + monomial.degree; //add

            multiplication(divisor, monomial, buffer);
            subtraction(remainder, buffer);
            
            for (int m = 0; m <= monomial.degree; m++) { //add
                monomial.terms[m] = 0;
            }
            //monomial�k�s
