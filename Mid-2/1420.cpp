...
void addition( Polynomial &addend, Polynomial adder )
{
    ...
    while (addend.degree > 1 && addend.terms[addend.degree - 1] == 0) {
        addend.degree--;
    }
...
void multiplication( Polynomial multiplicand, Polynomial multiplier, Polynomial &product )
{
    ...
    while (product.degree > 1 && product.terms[product.degree - 1] == 0) {
        product.degree--;
    }
...
void division( Polynomial dividend, Polynomial divisor, Polynomial &quotient, Polynomial &remainder )
{
    ...
    buffer.terms[remainder.degree];     // modify
    monomial.terms[quotient.degree];    // modify

    if (dividend.degree >= divisor.degree) {
        for (int ix = dividend.degree; ix >= 0; ix--) {
                quotient.terms[ix - divisor.degree] = dividend.terms[ix] / divisor.terms[divisor.degree];   // remainder[remainderDegree]/ divisor[divisorDegree]
                ...
                delete[]buffer.terms;   // delete
                delete[]monomial.terms; // delete
        }
    }
    
    while (quotient.degree > 1 && quotient.terms[quotient.degree - 1]) {
        quotient.degree--;
    }

    while (remainder.degree > 1 && remainder.terms[quotient.degree - 1]) {
        remainder.degree--;
    }
