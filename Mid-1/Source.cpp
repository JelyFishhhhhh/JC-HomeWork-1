...
void addition(int addend[], int adder[], int& addendDegree, int adderDegree)
{
    ...
    if (addendDegree < adderDegree) {
        for (int j = 0; j <= adderDegree; j++) {
            addendDegree = adderDegree;
            addend[j] += adder[j];
        }
    }
    while (addend[addendDegree] == 0 && addendDegree > 0) {
        addendDegree--;
    }
    ...
    int multiplicandDegree, int multiplierDegree, int& productDegree)
{
    ...
    productDegree = multiplicandDegree + multiplierDegree;//add
    /*while (product[productDegree] == 0 && productDegree > 0) {
        productDegree--;
    }*/
    ...
    int dividendDegree, int divisorDegree, int& quotientDegree, int& remainderDegree)
{
    ...
    // quotientDegree = dividendDegree - divisorDegree; deleted
    for (int i = 0; i <= dividendDegree; i++) {
        remainder[i] = dividend[i];
    }
    remainderDegree = dividendDegree;
    // int monomial[10000], buffer[10000];
    // int monomialDegree;                              deleted

    for (int j = quotientDegree; j >= 0; j--) {
        int monomial[10000]={}, buffer[10000]={};      // added
        int monomialDegree;
        quotient[j] = remainder[remainderDegree] / divisor[divisorDegree];
        monomial[j] = quotient[j];
        monomialDegree = j;
        int bufferDegree = divisorDegree + monomialDegree;

        multiplication(divisor, monomial, buffer, divisorDegree, monomialDegree, bufferDegree);
        subtraction(remainder, buffer, remainderDegree, bufferDegree);
        /*if (remainderDegree < quotientDegree) {
            break;
        }*/
        // memset(monomial, 0, monomialDegree);         deleted
        // memset(buffer, 0, bufferDegree);
        remainderDegree--;
    }
    remainderDegree++;
    if (quotientDegree != 0 && quotient[quotientDegree] == 0)
