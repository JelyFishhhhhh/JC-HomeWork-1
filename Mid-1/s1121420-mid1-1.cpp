...
void addition( int addend[], int adder[], int &addendDegree, int adderDegree )
{
    ...
    //added end
    if (adderDegree > addendDegree)addendDegree = adderDegree;
                     ...
                     int multiplicandDegree, int multiplierDegree, int &productDegree )
{
    ...
    for (int ix = 0; ix <= multiplicandDegree; ix++) {
        for (int iy = 0; iy <= multiplierDegree; iy++) {
            product[ix + iy] += multiplicand[ix] * multiplier[iy];
            /*if (multiplicand[ix++] == 0 || multiplier[iy++] == 0) {
                product[ix + iy] = 0;  
            }deleted*/
        }
    }
               ...
               int dividendDegree, int divisorDegree, int &quotientDegree, int &remainderDegree )
{
   ...
   //quotientDegree = remainderDegree; //deleted
   /*monomialDegree = remainderDegree - divisorDegree;
   multiplication(divisor, quotient, buffer, divisorDegree, quotientDegree, bufferDegree);  deleted*/
   // added start
   for (int i = quotientDegree; i >= 0; i--) {
       int monomialDegree; int monomial[200] = {}; int bufferDegree= remainderDegree; //added
       monomialDegree = i;
       quotient[i] = remainder[remainderDegree] / divisor[divisorDegree];
